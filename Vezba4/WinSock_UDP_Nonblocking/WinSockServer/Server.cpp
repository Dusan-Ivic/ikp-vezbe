...
int main(int argc,char* argv[])
{
    ...
    while(1)
    {
        ...
        if (accessBuffer[0] == 'S' || accessBuffer[0] == 's') {
            break;
        }

        struct Operation* operation = (struct Operation *)accessBuffer;

        int result = 0;
        switch (operation->sign) {
        case '+':
            result = (operation->operand1 - '0') + (operation->operand2 - '0');
            break;
        case '-':
            result = (operation->operand1 - '0') - (operation->operand2 - '0');
            break;
        case '^':
            result = pow((operation->operand1 - '0'), (operation->operand2 - '0'));
            break;
        }

        char outgoingBuffer[OUTGOING_BUFFER_SIZE];
        memset(outgoingBuffer, 0, OUTGOING_BUFFER_SIZE);

        itoa(result, outgoingBuffer, 10);
